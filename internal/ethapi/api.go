...
import (
	...
	"github.com/ubetprotocol/go-ubetprotocol/accounts/scwallet"
...
// PublicUbetprotocolAPI provides an API to access Ubetprotocol related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicUbetprotocolAPI struct {
	b Backend
}

// NewPublicUbetprotocolAPI creates a new Ubetprotocol protocol API.
func NewPublicUbetprotocolAPI(b Backend) *PublicUbetprotocolAPI {
	return &PublicUbetprotocolAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicUbetprotocolAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	price, err := s.b.SuggestPrice(ctx)
	return (*hexutil.Big)(price), err
}

// ProtocolVersion returns the current Ubetprotocol protocol version this node supports
func (s *PublicUbetprotocolAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicUbetprotocolAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Ubetprotocol ECDSA signature for:
// keccack256("\x19Ubetprotocol Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/ubetprotocol/go-ubetprotocol/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Ubetprotocol Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/ubetprotocol/go-ubetprotocol/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Ubetprotocol signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Ubetprotocol blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Ubetprotocol blockchain API.
...
//
// https://github.com/ubetprotocol/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of Ubetprotocol APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Ubetprotocol APIs exposed over the private
// debugging endpoint.
...
// Version returns the current ubetprotocol protocol version.
