...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.ubetprotocol", new NodeConfig());
			node.start();
			...
			UbetprotocolClient ec = node.getUbetprotocolClient();
	...
	// signatures too. Regression test for go-ubetprotocol issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ubetprotocol/gubettest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ubetprotocol", "github.com/ubetprotocol/go-ubetprotocol/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ubetprotocol", "gubettest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.ubetprotocol.gubettest"
