...
import (
	...
	"github.com/ubetprotocol/go-ubetprotocol/eth/fetcher"
...
// NewProtocolManager returns a new Ubetprotocol sub protocol manager. The Ubetprotocol sub protocol manages peers capable
// with the Ubetprotocol network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Ubetprotocol peer", "peer", id)

	// Unregister the peer from the downloader and Ubetprotocol peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Ubetprotocol protocol")
	...
	log.Info("Ubetprotocol protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Ubetprotocol peer connected", "name", p.Name())

	// Execute the Ubetprotocol handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Ubetprotocol handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Ubetprotocol peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Ubetprotocol message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Ubetprotocol sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Ubetprotocol network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
