...
type ProtocolManager struct {
	...
	client    bool   // The indicator whubetcoin the node is light client
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Ubetprotocol protocol")
	...
	log.Info("Light Ubetprotocol protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Ubetprotocol peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Ubetprotocol handshake failed", "err", err)
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Ubetprotocol peer registration failed", "err", err)
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			...
			p.Log().Debug("Light Ubetprotocol message handling failed", "err", err)
			if p.fcServer != nil {
				p.fcServer.DumpLogs()
			}
			return err
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Ubetprotocol message arrived", "code", msg.Code, "bytes", msg.Size)
