...
// Package utils contains internal helper functions for go-ubetprotocol commands.
package utils
...
import (
	...
	"github.com/ubetprotocol/go-ubetprotocol/common/fdlimit"
	...
	"github.com/ubetprotocol/go-ubetprotocol/dashboard"
	...
	"github.com/ubetprotocol/go-ubetprotocol/graphql"
	"github.com/ubetprotocol/go-ubetprotocol/les"
	"github.com/ubetprotocol/go-ubetprotocol/log"
	"github.com/ubetprotocol/go-ubetprotocol/metrics"
	"github.com/ubetprotocol/go-ubetprotocol/metrics/influxdb"
	...
	"github.com/ubetprotocol/go-ubetprotocol/p2p/netutil"
...
var (
	...
	MinerUbetcoinbaseFlag = cli.StringFlag{
		Name:  "miner.ubetcoinbase",
		Usage: "Public address for block mining rewards (default = first account)",
		Value: "0",
	}
	MinerLegacyUbetcoinbaseFlag = cli.StringFlag{
		Name:  "ubetcoinbase",
		Usage: "Public address for block mining rewards (default = first account, deprecated, use --miner.ubetcoinbase)",
		Value: "0",
	}
	...
	BootnodesFlag = cli.StringFlag{
		...
		Value: "enode://15c4d930d3ef822734bd7357974fd9b73161a756c7960228b74d7feba90738e176f076a91658358a461a91b6bbf8cebda0d5f3bbea2339717d7130a33a828fab@138.68.6.61:30303",
	}
...
// setUbetcoinbase retrieves the ubetcoinbase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func setUbetcoinbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *eth.Config) {
	// Extract the current ubetcoinbase, new flag overriding legacy one
	var ubetcoinbase string
	if ctx.GlobalIsSet(MinerLegacyUbetcoinbaseFlag.Name) {
		ubetcoinbase = ctx.GlobalString(MinerLegacyUbetcoinbaseFlag.Name)
	}
	if ctx.GlobalIsSet(MinerUbetcoinbaseFlag.Name) {
		ubetcoinbase = ctx.GlobalString(MinerUbetcoinbaseFlag.Name)
	}
	// Convert the ubetcoinbase into an address and configure it
	if ubetcoinbase != "" {
		if ks != nil {
			account, err := MakeAddress(ks, ubetcoinbase)
			if err != nil {
				Fatalf("Invalid miner ubetcoinbase: %v", err)
			}
			cfg.Miner.Ubetcoinbase = account.Address
		} else {
			Fatalf("No ubetcoinbase configured")
		}
	}
}
...
// RegisterEthService adds an Ubetprotocol client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Ubetprotocol service: %v", err)
	}
}
...
// RegisterEthStatsService configures the Ubetprotocol Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.Ubetprotocol
		ctx.Service(&ethServ)
		...
		Fatalf("Failed to register the Ubetprotocol Stats service: %v", err)
	}
}
...
func RegisterGraphQLService(stack *node.Node, endpoint string, cors, vhosts []string, timeouts rpc.HTTPTimeouts) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		return nil, errors.New("no Ubetprotocol service")
	}); err != nil {
