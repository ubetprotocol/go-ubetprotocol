...
import (
	...
	"github.com/ubetprotocol/go-ubetprotocol/accounts/usbwallet/trezor"
...
type trezorDriver struct {
	...
	pinwait        bool          // Flags whubetcoin the device is waiting for PIN entry
	passphrasewait bool          // Flags whubetcoin the device is waiting for passphrase entry
...
// Status implements accounts.Wallet, always whubetcoin the Trezor is opened, closed
// or whubetcoin the Ubetprotocol app was not started on it.
...
// Ubetprotocol address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.UbetprotocolAddress)
	if _, err := w.trezorExchange(&trezor.UbetprotocolGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.UbetprotocolSignTx{
	...
	response := new(trezor.UbetprotocolTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.UbetprotocolTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
