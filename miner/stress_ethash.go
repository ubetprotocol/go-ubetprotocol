...
func main() {
	...
	for _, node := range nodes {
		...
		if err := node.Service(&ubetprotocol); err != nil {
			panic(err)
		}
		if err := ubetprotocol.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var ubetprotocol *eth.Ubetprotocol
		if err := nodes[index%len(nodes)].Service(&ubetprotocol); err != nil {
			panic(err)
		}
		...
		if err := ubetprotocol.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := ubetprotocol.TxPool().Stats(); pend > 2048 {
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the Ubetprotocol node
	...
	// Start the node and configure a full Ubetprotocol node on it
