...
const (
	...
	URN_WANUbetcoinnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANUbetcoinnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Ubetcoinnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Ubetcoinnet
//
...
// WANUbetcoinnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANUbetcoinnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANUbetcoinnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANUbetcoinnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANUbetcoinnetLinkConfig1Clients() (clients []*WANUbetcoinnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANUbetcoinnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANUbetcoinnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANUbetcoinnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANUbetcoinnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANUbetcoinnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANUbetcoinnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANUbetcoinnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANUbetcoinnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANUbetcoinnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANUbetcoinnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANUbetcoinnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANUbetcoinnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANUbetcoinnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANUbetcoinnetLinkConfig1 {
	clients := make([]*WANUbetcoinnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANUbetcoinnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewUbetcoinnetLinkStatus: allowed values: Up, Down
func (client *WANUbetcoinnetLinkConfig1) GetUbetcoinnetLinkStatus() (NewUbetcoinnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewUbetcoinnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANUbetcoinnetLinkConfig_1, "GetUbetcoinnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewUbetcoinnetLinkStatus, err = soap.UnmarshalString(response.NewUbetcoinnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
