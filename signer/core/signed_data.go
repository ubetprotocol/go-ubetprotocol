...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useUbetprotocolV = true // Default to use V = 27 or 28, the legacy Ubetprotocol format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useUbetprotocolV, err
	}
	...
	case ApplicationClique.Mime:
		// Clique is the Ubetprotocol PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useUbetprotocolV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
		...
		useUbetprotocolV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: cliqueRlp, Message: message, Hash: sighash}
	default: // also case TextPlain.Mime:
		// Calculates an Ubetprotocol ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}Ubetprotocol Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useUbetprotocolV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
	...
	return req, useUbetprotocolV, nil
