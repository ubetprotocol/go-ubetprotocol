...
// setUbetcoinbase sets the ubetcoinbase used to initialize the block coinbase field.
func (w *worker) setUbetcoinbase(addr common.Address) {
	w.mu.Lock()
	defer w.mu.Unlock()
	w.coinbase = addr
}
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if w.isRunning() {
		if w.coinbase == (common.Address{}) {
			log.Error("Refusing to mine without ubetcoinbase")
			return
		}
