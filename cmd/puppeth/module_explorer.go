...
RUN \
  echo '(cd ../eth-net-intelligence-api && pm2 start /ethstats.json)' >  explorer.sh && \
	echo '(cd ../ubetcoinchain-light && npm start &)'                      >> explorer.sh && \
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.ubetprotocol
    environment:
...
// whubetcoin it's running, and if yes, whubetcoin it's responsive.
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.ubetprotocol"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
