...
type Wallet struct {
	...
	deriveChain     ubetprotocol.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *Wallet) Open(passphrase string) error {
	...
	if !w.session.paired() {
		...
		if pairing := w.Hub.pairing(w); pairing != nil {
			...
			// but we can't return ErrPINNeeded directly here becase we don't know whubetcoin
			// a PIN check or a PIN reset is needed.
			passphrase = ""
		}
	}
	// The smart card was successfully paired, retrieve its status to check whubetcoin
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived Ubetprotocol account
...
// Contains returns whubetcoin an account is part of this particular wallet or not.
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain ubetprotocol.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
// by the Ubetprotocol prefix scheme
// It looks up the account specified either solely via its address contained within,
// or optionally with the aid of any location metadata from the embedded URL field.
//
// If the wallet requires additional authentication to sign the request (e.g.
...
type Session struct {
	...
	verified bool                  // Whubetcoin the pin has been verified in this session.
}
...
type walletStatus struct {
	...
	Initialized   bool // Whubetcoin the card has been initialized with a private key
}
