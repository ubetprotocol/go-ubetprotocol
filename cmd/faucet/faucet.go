...
// faucet is a Ubetcoin faucet backed by a light client.
package main
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Ubetprotocol protocol")
	statsFlag   = flag.String("ethstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Ubetcoins to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Ubetprotocol and the faucet")
)

var (
	ubetcoin = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Ubetcoins", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Ubetprotocol address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Ubetprotocol light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Ubetprotocol protocol stack
	client *ethclient.Client   // Client connection to the Ubetprotocol chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Ubetprotocol light client protocol
	...
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightUbetprotocol
			ctx.Service(&serv)
...
// close terminates the Ubetprotocol connection and tears down the faucet.
...
// apiHandler handles requests for Ubetcoin grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, ubetcoin),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Ubetprotocol address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/ubetprotocol/go-ubetprotocol/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), ubetcoin)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, ubetcoin)
			peers := f.stack.Server().PeerCount()
...
// the username, avatar URL and Ubetprotocol address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Ubetprotocol address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Ubetprotocol address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Ubetprotocol address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Ubetprotocol address,
