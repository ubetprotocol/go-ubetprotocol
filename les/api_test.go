...
import (
	...
	"github.com/ubetprotocol/go-ubetprotocol/p2p/simulations"
	"github.com/ubetprotocol/go-ubetprotocol/p2p/simulations/adapters"
...
// similar requests at the maximum allowed rate and the test verifies whubetcoin the
...
func newLesServerService(ctx *adapters.ServiceContext) (node.Service, error) {
	...
	ubetprotocol, err := eth.New(ctx.NodeContext, &config)
	if err != nil {
		return nil, err
	}

	server, err := NewLesServer(ubetprotocol, &config)
	if err != nil {
		return nil, err
	}
	ubetprotocol.AddLesServer(server)
	return ubetprotocol, nil
}
