...
import (
	...
	math2 "github.com/ubetprotocol/go-ubetprotocol/common/math"
...
// newAlethGenesisSpec converts a go-ubetprotocol genesis block into a Aleth-specific
// chain specification format.
func newAlethGenesisSpec(network string, genesis *core.Genesis) (*alethGenesisSpec, error) {
	// Only ethash is currently supported between go-ubetprotocol and aleth
...
// newParityChainSpec converts a go-ubetprotocol genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-ubetprotocol and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	// https://github.com/ubetprotocol/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/ubetprotocol/EIPs/blob/master/EIPS/eip-607.md
	...
	spec.Genesis.Seal.Ubetprotocol.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.Ubetprotocol.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.Ubetprotocol.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// pyUbetprotocolGenesisSpec represents the genesis specification format used by the
// Python Ubetprotocol implementation.
type pyUbetprotocolGenesisSpec struct {
	Nonce      hexutil.Bytes     `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyUbetprotocolGenesisSpec converts a go-ubetprotocol genesis block into a Parity specific
// chain specification format.
func newPyUbetprotocolGenesisSpec(network string, genesis *core.Genesis) (*pyUbetprotocolGenesisSpec, error) {
	// Only ethash is currently supported between go-ubetprotocol and pyubetprotocol
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyUbetprotocolGenesisSpec{
