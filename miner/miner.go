...
// Package miner implements Ubetprotocol block creation and mining.
package miner
...
type Config struct {
	Ubetcoinbase common.Address `toml:",omitempty"` // Public address for block mining rewards (default = first account)
...
func (self *Miner) SetUbetcoinbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setUbetcoinbase(addr)
}
