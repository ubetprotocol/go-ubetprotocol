...
// nodeDockerfile is the Dockerfile required to run an Ubetprotocol node.
var nodeDockerfile = `
FROM ubetprotocol/client-go:latest
...
RUN \
  echo 'gubet --cache 512 init /genesis.json' > gubet.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.ubetprotocol/keystore/ && cp /signer.json /root/.ubetprotocol/keystore/' >> gubet.sh && \{{end}}
	echo $'exec gubet --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Ubetcoinbase}}--miner.ubetcoinbase {{.Ubetcoinbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gubet.sh

ENTRYPOINT ["/bin/sh", "gubet.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Ubetprotocol node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.ubetprotocol{{if .Ethashdir}}
...
// deployNode deploys a new Ubetprotocol node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.ubetcoinbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Ubetcoinbase": config.ubetcoinbase,
...
func (info *nodeInfos) Report() map[string]string {
	...
	if info.gasTarget > 0 {
		...
		if info.ubetcoinbase != "" {
			// Ethash proof-of-work miner
			report["Ethash directory"] = info.ethashdir
			report["Miner account"] = info.ubetcoinbase
		}
		if info.keyJSON != "" {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.ubetprotocol"],
		ethashdir:  infos.volumes["/root/.ethash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		ethstats:   infos.envvars["STATS_NAME"],
		ubetcoinbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasLimit:   gasLimit,
